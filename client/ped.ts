...
export function ChangeComponents(shouldChangeModel?: boolean) {
    ...
    if(outfits[Character.outfit]) {
        Object.entries(outfits[Character.outfit]).forEach((entry) => {
            ...
            SetPedComponentVariation(PlayerPedId(), component, drawable, texture, 2);
        });
    }
...
export function RefreshModel(force: boolean) {
    const immediate = setImmediate(() => {
        const Character = store.character;
        //replaces the player's current model if it wasn't the supposed one already
		if(force || GetEntityModel(PlayerPedId()) !== store.mdhash) {
			while(!HasModelLoaded(store.mdhash)) {
				RequestModel(store.mdhash)
				Wait(0)
            }
			SetPlayerModel(PlayerId(), store.mdhash);
			SetPedHeadBlendData(PlayerPedId(), Character["mom"], Character["dad"], 0, Character["mom"], Character["dad"], 0, Character["resemblance"], Character["skintone"], 0, true);
			ChangeComponents()
        }
